...
void SoftDemo::createStack(btCollisionShape* boxShape, float halfCubeSize, int size, float zPos)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		for (int j = 0; j < rowSize; j++)
		{
			...
			btRigidBody* body = 0;
			body = m_physics->createRigidBody(mass, trans, boxShape);
		}
	}
}
...
static void Ctor_RbUpStack(SoftDemo* pdemo, int count)
{
	...
	for (int i = 0; i < count; ++i)
	{
		...
		pdemo->m_physics->createRigidBody(mass, startTransform, shape[i % nshapes]);
...
static void Ctor_BigBall(SoftDemo* pdemo, btScalar mass = 10)
{
	...
	pdemo->m_physics->createRigidBody(mass, startTransform, new btSphereShape(3));
}
...
static btRigidBody* Ctor_BigPlate(SoftDemo* pdemo, btScalar mass = 15, btScalar height = 4)
{
	...
	btRigidBody* body = pdemo->m_physics->createRigidBody(mass, startTransform, new btBoxShape(btVector3(5, 1, 5)));
	body->setFriction(1);
	return (body);
}
...
static void Ctor_LinearStair(SoftDemo* pdemo, const btVector3& org, const btVector3& sizes, btScalar angle, int count)
{
	...
	for (int i = 0; i < count; ++i)
	{
		...
		btRigidBody* body = pdemo->m_physics->createRigidBody(0, startTransform, shape);
		body->setFriction(1);
	}
}
...
static void Init_RopeAttach(SoftDemo* pdemo)
{
	...
	btRigidBody* body = pdemo->m_physics->createRigidBody(50, startTransform, new btBoxShape(btVector3(2, 6, 2)));
...
static void Init_ClothAttach(SoftDemo* pdemo)
{
	...
	btRigidBody* body = pdemo->m_physics->createRigidBody(20, startTransform, new btBoxShape(btVector3(s, 1, 3)));
...
static void Init_Impact(SoftDemo* pdemo)
{
	...
	pdemo->m_physics->createRigidBody(10, startTransform, new btBoxShape(btVector3(2, 2, 2)));
}
...
static void Init_CapsuleCollision(SoftDemo* pdemo)
{
	...
	btRigidBody* body = pdemo->m_physics->createRigidBody(0, startTransform, capsuleShape);
...
#endif
	btRigidBody* body = pdemo->m_physics->createRigidBody(10, startTransform, shape);
	body->setFriction(1);
	btDynamicsWorld* world = pdemo->m_physics->getDynamicsWorld();
...
static void Init_ClusterRobot(SoftDemo* pdemo)
{
	...
	btRigidBody* prb = pdemo->m_physics->createRigidBody(50, btTransform(btQuaternion(0, 0, 0), ctr + btVector3(0, 5, 0)), pshp);
	...
	btRigidBody* pgrn;
	pgrn = pdemo->m_physics->createRigidBody(0, btTransform(btQuaternion(0, -SIMD_HALF_PI / 2, 0), btVector3(0, 0, 0)), pbox);
...
void SoftDemo::initPhysics()
{
	...
	auto broadphase = new btAxisSweep3(worldAabbMin, worldAabbMax, maxProxies);
	...
	m_physics->m_collisionShapes.push_back(groundBox);
	...
	m_physics->m_collisionShapes.push_back(cylinderCompound);

	m_softBodyWorldInfo.m_dispatcher = m_physics->getDispatcher();
	...
	////////////////////////////
	m_softBodyWorldInfo.m_broadphase = m_physics->getBroadphase();
	...
	if (current_demo > 19)
	{
		newOb->setCollisionShape(m_physics->m_collisionShapes[0]);
	}
	else
	{
		newOb->setCollisionShape(m_physics->m_collisionShapes[1]);
	}
